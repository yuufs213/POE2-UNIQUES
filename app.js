...
function updateDialogButton() {
  ...
  dialogToggle.textContent = collected.has(currentDialogItem.id) ? "Mark Missing" : "Mark Owned";
}
...
function importProgress(file) {
  ...
  reader.addEventListener("load", () => {
    try {
      ...
      collected = new Set(incoming.filter((id) => validIds.has(id)));
      saveCollected();
      render();
    } catch {
      return;
...
document.querySelector("#markVisibleButton").addEventListener("click", () => {
  filteredItems().forEach((item) => collected.add(item.id));
  saveCollected();
  render();
});

document.querySelector("#clearVisibleButton").addEventListener("click", () => {
  filteredItems().forEach((item) => collected.delete(item.id));
  saveCollected();
  render();
});
...
dialogToggle.addEventListener("click", () => {
  if (currentDialogItem) toggleItem(currentDialogItem.id);
});
